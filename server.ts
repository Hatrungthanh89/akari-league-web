...
// 1. API: Generate Match Report (Match Analysis Article)
...
// 2. API: Generate Round Summary (Compilation News with 3 Commentators)
...
Yêu cầu xuất ra định dạng JSON chuẩn theo schema sau:
{
  ...
  "comments": [
    ...
    console.error("Round Summary Generation Error:", error);
    return res.status(500).json({ error: error.message || "Không thể tự động tổng hợp vòng qua AI" });
  }
});

// 3. Vite development server / production routing integration
async function boostServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Express server routing matches and AI summaries on port ${PORT}`);
  });
}
