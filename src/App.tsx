...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubNews = onSnapshot(collection(db, 'news'), (snap) => {
      // Sort news by date descending roughly, or just keep as is (since IDs are generated by time often)
      const newsData = snap.docs.map(d => d.data() as News);
      // Sort by some logic if needed, but we'll just reverse it so latest is first
  ...
  const renderTabContent = () => {
    if (isSyncing) return <div className="p-8 text-center animate-pulse">Đang kết nối Firebase...</div>;
    switch (currentTab) {
      case 'standings':
        return (
          <>
